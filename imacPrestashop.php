	...
	*/
				...
				<div id="probando_bd" style="display:none;">
					<?php												
						...
						$sql ="
						  ...
						  IF(pr.reduction_type = 'amount', pr.reduction, '') AS 'Discount_amount', pr.reduction_tax as reduction_offer, s.quantity						  
						...
						{		
							...
							{				
								...
								$content.="<li>";
								if ($url_base==0){
									$content.="<a title='".utf8_encode($registrosql['name'])."' href='".$url_tienda.$lang_txt.$row->id_product."-".$registrosql['link_rewrite'].".html' target='blank_'>";								
								}else{
									$content.="<a title='".utf8_encode($registrosql['name'])."' href='".$url_tienda.$lang_txt.$registrosql['link_rewrite']."-".$row->id_product.".html' target='blank_'>";								
								}
								...
								if ($url_base==0){
									$content.="<br><a class='product_name' title='".utf8_encode($registrosql['name'])."' href='".$url_tienda.$lang_txt.$row->id_product."-".$registrosql['link_rewrite'].".html' target='blank_'>";
								}else{
									$content.="<br><a class='product_name' title='".utf8_encode($registrosql['name'])."' href='".$url_tienda.$lang_txt.$registrosql['link_rewrite']."-".$row->id_product.".html' target='blank_'>";
								}
								...
								if ($Discount_amount!=0){
									$content.="<span class='offer'>".$preu."</span><span class='price'>".$preu_dto." Euros</span><br>";
								}else{
									$content.="<span class='price'>".$preu." Euros</span><br>";
								}													
								$content.="</li>";			
								$i++;
							}	
	...
	function imacPrestashop_shortcode_productos($atts) {		        
		...
		$sql ="
			SELECT p.*, pa.id_product_attribute, pl.description, pl.description_short, pl.available_now, pl.available_later, pl.link_rewrite, pl.meta_description,
		 	pl.meta_keywords, pl.meta_title, pl.name, i.id_image, il.legend, 	p.price as precio_base,
		  ROUND(p.price * (COALESCE(ptx.rate, 0) / 100 + 1), 2) AS 'regular_price', ptx.rate as iva,
		  IF(pr.reduction_type = 'amount', pr.reduction, '') AS 'Discount_amount', pr.reduction_tax as reduction_offer
		  FROM $category_product cp 
		  LEFT JOIN $product p ON p.id_product = cp.id_product 
		  LEFT JOIN $product_attribute pa ON (p.id_product = pa.id_product AND default_on = 1) 
		  LEFT JOIN $category_lang cl ON (p.id_category_default = cl.id_category AND cl.id_lang = '.$id_idioma.') 
	...
	class imacPrestashop_encriptacion{				
		public static function encriptar($cadena){	
			...
			$iv = '12as16as78as12as';			
	    $encrypted = openssl_encrypt($cadena,'AES-256-CBC',$key,0,$iv_a); 
	    return $encrypted; 
		} 
		public static function desencriptar($cadena){  		
			...
			$key ='asasest&A2oeds3-asdwas23'.$wpdb->base_prefix.'Acunt#33ddasd_asextod2Dseprueba31';	
			$iv = '12as16as78as12as';	   	
	   	$decrypted = openssl_decrypt($cadena,'AES-256-CBC',$key,0,$iv_a); 	
	    return $decrypted;
		}
	}
?>